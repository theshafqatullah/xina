...
const VALUES = [
  { icon: Target, title: "Universal builder", desc: "Zinaplus supports 10+ database engines \u2014 from PostgreSQL and MySQL to MongoDB, Appwrite, and Firebase. One tool for every data layer.", accent: "from-emerald-500 to-blue-500" },
  ...
  { icon: Heart, title: "Open & transparent", desc: "Open-source core with a clear roadmap. Every release is documented in the changelog. Community contributions are welcome.", accent: "from-emerald-500 to-emerald-500" },
];
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute right-0 top-60 h-[400px] w-[400px] rounded-full bg-emerald-600/[0.04] blur-[120px]" />
      </div>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="grid items-center gap-10 md:grid-cols-2">
          <div>
            <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Mission</span>
            <h2 className="mb-3 text-xl font-bold text-zinc-900 md:text-2xl">Our mission</h2>
          ...
          <div className="flex items-center justify-center">
            <div className="group relative h-52 w-52">
              <div className="absolute inset-0 rounded-3xl border border-zinc-200 bg-zinc-50 transition-all group-hover:border-zinc-300" />
              <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full bg-emerald-600/[0.08] blur-[40px]" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <div className="mx-auto mb-3 text-emerald-600">
                    <Database size={32} />
                  </div>
              <p className="text-sm font-semibold text-zinc-900">Universal Database Design</p>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-14 text-center">
          <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Values</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">What we believe</h2>
          <p className="mt-3 text-sm text-zinc-500">The principles that guide every decision.</p>
        </motion.div>
        <div className="grid gap-5 sm:grid-cols-2">
          {VALUES.map((v, i) => (
            <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: i * 0.06 }} className="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 p-7 transition-all hover:border-zinc-300">
              ...
              <div className="mb-4 text-emerald-600"><v.icon size={22} /></div>
              <h3 className="mb-2 text-[15px] font-semibold text-zinc-900">{v.title}</h3>
      ...
      <section className="relative mx-auto max-w-3xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-14 text-center">
          <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">History</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">Project timeline</h2>
        </motion.div>
        <div className="space-y-0">
          {TIMELINE.map((t, i) => (
            <motion.div key={t.date} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.05 }} className="relative border-l-2 border-zinc-200 pb-10 pl-8 last:pb-0">
              <div className="absolute -left-2 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-emerald-500/40 bg-white ring-4 ring-[#09090b]" />
              <span className="block text-xs font-semibold text-emerald-600">{t.date}</span>
              <h3 className="mt-1 text-[15px] font-semibold text-zinc-900">{t.title}</h3>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-14 text-center">
          <span className="mb-3 inline-block rounded-full bg-amber-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-amber-600">Stack</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">Tech stack</h2>
          <p className="mt-3 text-sm text-zinc-500">The technologies powering Zinaplus under the hood.</p>
        </motion.div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {TECH_STACK.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.04 }} className="group flex items-start gap-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-5 transition-all hover:border-zinc-300">
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-600/[0.12] via-emerald-600/[0.06] to-transparent p-10 text-center md:p-16">
          ...
          <div className="relative">
            <h2 className="text-2xl font-bold text-zinc-900 md:text-4xl">Try Zinaplus today</h2>
            <p className="mt-3 text-sm text-zinc-600 md:text-base">Free, open, and built for every database developer.</p>
