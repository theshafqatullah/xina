...
const VALUES = [
  { icon: Target, title: "Purpose-built", desc: "Xina is designed exclusively for Appwrite. Every feature maps directly to the Appwrite databases API — no generic diagramming layers.", accent: "from-indigo-500 to-blue-500" },
  { icon: Zap, title: "Developer-first", desc: "Built by developers, for developers. Fast setup, keyboard shortcuts, code generation, and zero friction between idea and implementation.", accent: "from-amber-500 to-orange-500" },
...
const TECH_STACK = [
  { name: "Next.js 16", desc: "App Router with React 19 and server components" },
  { name: "TypeScript", desc: "End-to-end type safety across the entire codebase" },
  { name: "Appwrite SDK", desc: "Official Node.js and Web SDKs for database operations" },
  { name: "Framer Motion", desc: "60fps animations for canvas interactions and page transitions" },
  { name: "Tailwind CSS 4", desc: "Utility-first styling with dark mode by default" },
  { name: "Lucide Icons", desc: "Clean, consistent icon set throughout the interface" },
];

const TIMELINE = [
  { date: "Mar 2025", title: "v0.1 — Initial release", desc: "Basic Appwrite connection, collection listing, and initial canvas rendering." },
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-16 pt-28 text-center md:pt-36">
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.08 } } }} className="flex flex-col items-center">
          ...
          <motion.h1 variants={fadeUp} transition={{ duration: 0.5 }} className="max-w-2xl text-3xl font-extrabold tracking-tight text-white md:text-5xl">
            The story behind{" "}
            <span className="bg-gradient-to-r from-indigo-400 via-violet-400 to-purple-400 bg-clip-text text-transparent">Xina</span>
          </motion.h1>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="grid items-center gap-10 md:grid-cols-2">
          <div>
            <span className="mb-3 inline-block rounded-full bg-indigo-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-indigo-400">Mission</span>
          ...
          <div className="flex items-center justify-center">
            <div className="group relative h-52 w-52">
              ...
              <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full bg-indigo-600/[0.08] blur-[40px]" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-indigo-600/10 ring-1 ring-indigo-500/20 text-indigo-400">
                    <Database size={28} />
                  </div>
                  <p className="text-sm font-semibold text-white">Visual Schema Design</p>
                  <p className="mt-0.5 text-[11px] text-zinc-500">for Appwrite</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {VALUES.map((v, i) => (
            <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: i * 0.06 }} className="group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] p-7 transition-all hover:border-white/[0.1]">
              ...
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-600/10 ring-1 ring-indigo-500/20 text-indigo-400 transition-all group-hover:bg-indigo-600/15"><v.icon size={19} /></div>
        ...
        <div className="space-y-0">
          {TIMELINE.map((t, i) => (
            <motion.div key={t.date} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.05 }} className="relative border-l-2 border-white/[0.06] pb-10 pl-8 last:pb-0">
              <div className="absolute -left-2 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-indigo-500/40 bg-[#09090b] ring-4 ring-[#09090b]" />
              <span className="block text-xs font-semibold text-indigo-400">{t.date}</span>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-32">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {TECH_STACK.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.04 }} className="group rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition-all hover:border-white/[0.1]">
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-indigo-600/[0.12] via-violet-600/[0.06] to-transparent p-10 text-center md:p-16">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-sm text-zinc-400 md:text-base">Free, open, and built for Appwrite developers.</p>
