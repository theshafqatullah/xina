...
const SECTIONS = [
  ...
  },
  {
    ...
    desc: "Zinaplus reads your Appwrite relationship attributes and automatically maps connections between collections. Two-way relationships show crow's foot notation; one-way relationships display directional arrows.",
    features: [
...
const COMPARISON = [
  { feature: "10+ database engines", zinaplus: true, generic: false },
  { feature: "AI schema generation", zinaplus: true, generic: false },
  { feature: "Drag & drop table builder", zinaplus: true, generic: false },
  { feature: "Auto relationship detection", zinaplus: true, generic: false },
  { feature: "Crow's foot ERD notation", zinaplus: true, generic: false },
  { feature: "One-click deploy to database", zinaplus: true, generic: false },
  { feature: "SQL + NoSQL + BaaS support", zinaplus: true, generic: false },
  { feature: "Free & open source", zinaplus: true, generic: false },
];

const fadeUp = { hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0 } };

const badgeColors: Record<string, string> = {
  purple: "border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-600",
  violet: "border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-600",
  emerald: "border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-600",
  amber: "border-amber-500/20 bg-amber-500/[0.06] text-amber-600",
};
...
function FeatureVisual({ type }: { type: string }) {
  const base = "relative h-56 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl shadow-black/20 md:h-72";
  ...
  if (type === "canvas") {
    return (
      <div className={base}>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.2 }} className="absolute left-6 top-6 h-20 w-32 rounded-lg border border-zinc-200 bg-white shadow-lg shadow-black/20">
          <div className="flex items-center gap-1.5 border-b border-zinc-200 bg-zinc-50 px-3 py-1.5"><Database size={9} className="text-emerald-600" /><span className="text-[9px] font-semibold text-zinc-900">Users</span></div>
          <div className="space-y-0.5 px-3 py-1 text-[8px] text-zinc-500"><div>name</div><div>email</div><div className="text-emerald-600">posts →</div></div>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.4 }} className="absolute bottom-8 right-6 h-20 w-32 rounded-lg border border-zinc-200 bg-white shadow-lg shadow-black/20">
          <div className="flex items-center gap-1.5 border-b border-zinc-200 bg-zinc-50 px-3 py-1.5"><Database size={9} className="text-emerald-600" /><span className="text-[9px] font-semibold text-zinc-900">Posts</span></div>
          <div className="space-y-0.5 px-3 py-1 text-[8px] text-zinc-500"><div>title</div><div>content</div><div className="text-emerald-600">← author</div></div>
        </motion.div>
  ...
  if (type === "relationships") {
    return (
      <div className={base + " flex items-center justify-center p-8"}>
        <div className="pointer-events-none absolute -left-10 -top-10 h-40 w-40 rounded-full bg-emerald-600/[0.06] blur-[60px]" />
        <div className="flex items-center gap-6">
          <motion.div initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.2 }} className="w-28 rounded-lg border border-zinc-200 bg-white shadow-lg shadow-black/20">
            <div className="border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-[10px] font-semibold text-zinc-900">Users</div>
            <div className="px-3 py-1.5 text-[9px] text-emerald-600">posts →</div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 }} className="w-28 rounded-lg border border-zinc-200 bg-white shadow-lg shadow-black/20">
            <div className="border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-[10px] font-semibold text-zinc-900">Posts</div>
            <div className="px-3 py-1.5 text-[9px] text-emerald-600">← author</div>
          </motion.div>
        </div>
        <div className="absolute bottom-6 rounded-full bg-emerald-500/[0.06] px-3 py-1 text-center text-[10px] font-medium text-emerald-600/60">Many-to-many (two-way)</div>
      </div>
    );
  }
      ...
      <div className={base + " p-6"}>
        ...
        <div className="space-y-2">
          {["users", "posts", "comments", "tags"].map((name, i) => (
            <motion.div key={name} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: 0.15 * i }} className="flex items-center gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-2.5 transition-colors hover:border-zinc-200">
              <Table2 size={12} className="text-emerald-600" />
              <span className="text-[12px] font-medium text-zinc-700">{name}</span>
              <span className="ml-auto rounded-full bg-zinc-100 px-2 py-0.5 font-mono text-[9px] text-zinc-400">{3 + i} attrs</span>
            </motion.div>
          ))}
        </div>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#0c0c0f] to-transparent" />
  ...
  return (
    <div className={base + " p-6"}>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-4">
        <div className="mb-3 flex items-center gap-2"><Database size={12} className="text-amber-600" /><span className="text-xs font-semibold text-white">Users</span><span className="ml-auto rounded-full bg-emerald-500/10 px-2 py-0.5 text-[8px] font-semibold text-emerald-600">Selected</span></div>
        <div className="space-y-1.5 text-[11px]">
          <div className="flex items-center justify-between rounded-md bg-zinc-50 px-2 py-1 text-zinc-600"><span>name</span><span className="rounded bg-emerald-500/10 px-1.5 py-0.5 font-mono text-[9px] text-emerald-500">string</span></div>
          <div className="flex items-center justify-between rounded-md px-2 py-1 text-zinc-600"><span>email</span><span className="rounded bg-sky-500/10 px-1.5 py-0.5 font-mono text-[9px] text-sky-500">email</span></div>
          <div className="flex items-center justify-between rounded-md bg-zinc-50 px-2 py-1 text-zinc-600"><span>isAdmin</span><span className="rounded bg-amber-500/10 px-1.5 py-0.5 font-mono text-[9px] text-amber-600">boolean</span></div>
          <div className="flex items-center justify-between rounded-md px-2 py-1 text-emerald-600"><span>posts</span><span className="rounded bg-emerald-500/10 px-1.5 py-0.5 font-mono text-[9px] text-emerald-600">relation</span></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute right-0 top-40 h-[400px] w-[400px] rounded-full bg-emerald-600/[0.05] blur-[120px]" />
      </div>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-16 pt-28 text-center md:pt-36">
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.08 } } }} className="flex flex-col items-center">
          ...
          <motion.h1 variants={fadeUp} transition={{ duration: 0.5 }} className="max-w-3xl text-3xl font-extrabold tracking-tight text-zinc-900 md:text-5xl">
            Built for{" "}
            <span className="text-emerald-600">every database</span>{" "}
            developer
          </motion.h1>
          <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-4 max-w-lg text-base text-zinc-600 md:text-lg">From PostgreSQL to MongoDB to Appwrite — every feature is designed around the databases you actually use.</motion.p>
        </motion.div>
      </section>
        ...
        <section key={section.title} className="relative mx-auto max-w-6xl px-6 pb-28">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className={`grid items-center gap-12 md:grid-cols-2 ${i % 2 === 1 ? "md:[direction:rtl]" : ""}`}>
            <div className={i % 2 === 1 ? "md:[direction:ltr]" : ""}>
              ...
              <h2 className="mb-3 text-xl font-bold text-zinc-900 md:text-2xl">{section.title}</h2>
              <p className="mb-6 text-[14px] leading-relaxed text-zinc-600">{section.desc}</p>
              <ul className="space-y-3">
                {section.features.map((f) => (
                  <li key={f.label} className="flex items-start gap-3 text-[13px] text-zinc-600">
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-14 text-center">
          <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Attribute Types</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">Universal column & attribute types</h2>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {ATTR_TYPES.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.04 }} className="group rounded-xl border border-zinc-200 bg-zinc-50 p-4 transition-all hover:border-zinc-300">
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="grid items-center gap-12 md:grid-cols-2">
          <div>
            <span className="mb-4 inline-block rounded-full border border-emerald-500/20 bg-emerald-500/[0.06] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Code Generation</span>
            <h2 className="mb-3 text-xl font-bold text-zinc-900 md:text-2xl">Multi-language Code Generation</h2>
            <p className="mb-6 text-[14px] leading-relaxed text-zinc-600">Select any collection and generate typed code in your language of choice. Interfaces, model classes, schema definitions — ready to copy.</p>
            <ul className="space-y-3">
              {["TypeScript interfaces & Zod schemas", "Python dataclasses & Pydantic models", "Dart, Kotlin, Swift, Go, Rust model classes", "PHP classes & Ruby structs"].map((t) => (
                <li key={t} className="flex items-center gap-3 text-[13px] text-zinc-600">
          ...
          <div>
            <div className="relative">
              ...
              <div className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl shadow-black/30">
                <div className="flex items-center gap-2 border-b border-zinc-200 bg-zinc-50 px-4 py-2.5">
                  <span className="rounded bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-600">TypeScript</span>
                  <span className="rounded bg-zinc-100 px-2 py-0.5 text-[10px] text-zinc-400">Python</span>
                  <span className="rounded bg-zinc-100 px-2 py-0.5 text-[10px] text-zinc-400">Dart</span>
                </div>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-10 text-center">
          <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Shortcuts</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">Canvas shortcuts</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
          {KEYBOARD_SHORTCUTS.map((s, i) => (
            <div key={s.desc} className={`flex items-center justify-between px-6 py-4 transition-colors hover:bg-zinc-50 ${i > 0 ? "border-t border-zinc-200" : ""}`}>
              <span className="text-[13px] text-zinc-600">{s.desc}</span>
              <div className="flex gap-1.5">
                {s.keys.map((k) => (
                  <kbd key={k} className="rounded-lg border border-zinc-200 bg-zinc-100 px-3 py-1 font-mono text-[11px] font-medium text-zinc-700 shadow-sm shadow-black/10">{k}</kbd>
                ))}
              </div>
            </div>
          ))}
      ...
      {/* ─── Zinaplus vs generic tools ─── */}
      <section className="relative mx-auto max-w-4xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-10 text-center">
          <span className="mb-3 inline-block rounded-full bg-emerald-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Comparison</span>
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-4xl">Zinaplus vs. generic diagramming tools</h2>
          <p className="mt-3 text-sm text-zinc-500">Purpose-built beats generic every time.</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
          <div className="grid grid-cols-[1fr_80px_80px] border-b border-zinc-200 bg-zinc-50 px-6 py-3.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-500">
            <span>Feature</span><span className="text-center text-emerald-600">Zinaplus</span><span className="text-center">Generic</span>
          </div>
          {COMPARISON.map((r, i) => (
            <div key={r.feature} className={`grid grid-cols-[1fr_80px_80px] px-6 py-3.5 transition-colors hover:bg-zinc-50 ${i > 0 ? "border-t border-zinc-200" : ""}`}>
              <span className="text-[13px] text-zinc-600">{r.feature}</span>
              <span className="flex justify-center">{r.zinaplus ? <CheckCircle2 size={15} className="text-emerald-600" /> : <span className="text-zinc-300">—</span>}</span>
              <span className="flex justify-center">{r.generic ? <CheckCircle2 size={15} className="text-emerald-600" /> : <X size={14} className="text-zinc-300/50" />}</span>
            </div>
          ))}
        </motion.div>
      </section>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-32">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="grid items-center gap-12 md:grid-cols-2 md:[direction:rtl]">
          <div className="md:[direction:ltr]">
            <span className="mb-4 inline-block rounded-full border border-emerald-500/20 bg-emerald-500/[0.06] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Export</span>
            <h2 className="mb-3 text-xl font-bold text-zinc-900 md:text-2xl">Export & Share</h2>
            <p className="mb-6 text-[14px] leading-relaxed text-zinc-600">Export your ERD as a high-resolution PNG or SVG image for documentation, presentations, or team discussions. Share schemas without requiring others to connect.</p>
            <ul className="space-y-3">
          ...
          <div className="md:[direction:ltr]">
            <div className="relative">
              <div className="pointer-events-none absolute -inset-2 rounded-3xl bg-gradient-to-b from-emerald-600/[0.05] to-transparent blur-xl" />
              <div className="relative h-56 overflow-hidden rounded-2xl border border-zinc-200 bg-white p-8 shadow-xl shadow-black/20 md:h-72">
                <div className="flex h-full flex-col items-center justify-center gap-5">
                  <div className="flex gap-4">
                    {[
                      { icon: Code2, label: "TypeScript", color: "text-emerald-600" },
                      { icon: FileCode2, label: "Python", color: "text-emerald-600" },
                      { icon: Download, label: "SVG", color: "text-emerald-600" },
                    ].map((item) => (
                      <motion.div key={item.label} whileHover={{ y: -2 }} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-center transition-all hover:border-zinc-300">
                        <item.icon size={22} className={`mx-auto mb-2 ${item.color}`} />
                        <span className="text-[10px] font-medium text-zinc-500">{item.label}</span>
                      </motion.div>
                    ))}
                  </div>
                  <span className="rounded-full bg-zinc-100 px-3 py-1 text-[11px] text-zinc-400">One-click export to multiple formats</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-600/[0.12] via-emerald-600/[0.06] to-transparent p-10 text-center md:p-16">
          ...
          <div className="relative">
            <h2 className="text-2xl font-bold text-zinc-900 md:text-4xl">See it in action</h2>
            <p className="mt-3 text-sm text-zinc-600 md:text-base">Pick your database engine, design your schema, and explore every feature.</p>
            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
              ...
              <Link href="/docs" className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-7 py-3.5 text-sm font-semibold text-zinc-900 transition-all hover:border-white/30 hover:bg-zinc-100">
                Read Docs
              </Link>
            </div>
          </div>
