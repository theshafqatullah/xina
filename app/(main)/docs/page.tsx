    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-12 pt-28 text-center md:pt-36">
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.08 } } }} className="flex flex-col items-center">
          ...
          <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-4 max-w-lg text-base text-zinc-400 md:text-lg">Connect Xina to any of 10+ database engines in under two minutes. Here&apos;s everything you need.</motion.p>
        </motion.div>
      </section>

      {/* Table of contents */}
      <section className="relative mx-auto max-w-3xl px-6 pb-12">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
          ...
          <ul className="space-y-2">
            {["Getting Started (5 steps)", "Key Concepts", "Canvas Controls", "Troubleshooting", "API Reference"].map((title, i) => (
              <li key={title} className="flex items-center gap-2.5 text-[13px] text-zinc-400 transition-colors hover:text-purple-400 cursor-default">
        ...
        <div className="space-y-0">
          {STEPS.map((s, i) => (
            <motion.div key={s.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: i * 0.05 }} className="relative border-l-2 border-white/[0.06] pb-10 pl-8 last:pb-0">
              <div className="absolute -left-[17px] top-0 flex h-8 w-8 items-center justify-center rounded-full bg-[#09090b] text-xs font-bold text-purple-400">{s.step}</div>
              <div className="flex items-center gap-2.5 pt-1">
                <span className="text-purple-400"><s.icon size={16} /></span>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-14 text-center">
          <span className="mb-3 inline-block rounded-full bg-purple-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-purple-400">Reference</span>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {TOPICS.map((t, i) => (
            <motion.div key={t.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.05 }} className="group rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition-all hover:border-white/[0.1]">
              <div className="mb-3 text-purple-400"><t.icon size={20} /></div>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {TROUBLESHOOTING.map((t, i) => (
            <motion.div key={t.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.35, delay: i * 0.05 }} className="group rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition-all hover:border-white/[0.1]">
              <div className="mb-3 text-amber-400"><t.icon size={20} /></div>
              <h3 className="mb-1.5 text-[15px] font-semibold text-white">{t.title}</h3>
              <p className="text-[13px] leading-relaxed text-zinc-500">{t.desc}</p>
            </motion.div>
          ))}
      ...
      <section className="relative mx-auto max-w-3xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          <div className="mb-10 text-center">
            <span className="mb-3 inline-block rounded-full bg-purple-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-purple-400">API</span>
            <div className="mt-2 flex items-center justify-center gap-2"><Terminal size={16} className="text-purple-400" /><h2 className="text-2xl font-bold text-white md:text-4xl">Appwrite API Reference</h2></div>
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02]">
            ...
            {API_REFERENCE.map((r, i) => (
              <div key={r.path + r.method} className={`flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-white/[0.02] ${i > 0 ? "border-t border-white/[0.04]" : ""}`}>
                <span className={`font-mono text-[10px] font-bold ${r.method === "GET" ? "text-emerald-400" : r.method === "POST" ? "text-blue-400" : "text-red-400"}`}>{r.method}</span>
...
API Key:    your-api-key`}</pre>
          ...
          <div className="flex items-center justify-center">
            <div className="group w-full max-w-xs rounded-2xl border border-white/[0.06] bg-white/[0.02] p-7 text-center transition-all hover:border-white/[0.1]">
              <div className="mx-auto mb-4 text-purple-400"><Database size={28} /></div>
              <p className="text-[15px] font-semibold text-white">10+ Database Engines</p>
      ...
      <section className="relative mx-auto max-w-4xl px-6 pb-28">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-purple-500/20 bg-gradient-to-br from-purple-600/[0.12] via-violet-600/[0.06] to-transparent p-10 text-center md:p-16">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-sm text-zinc-400 md:text-base">Open the studio, pick your database engine, and see your schema come to life.</p>
